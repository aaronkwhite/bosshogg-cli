...
// ── 8. tiles move: hits /move_tile/ endpoint with {tile:{id,layouts:{sm}}} ────

#[tokio::test]
async fn dashboard_tiles_move_calls_move_tile_endpoint() {
    use wiremock::matchers::body_partial_json;

    let h = TestHarness::new().await;

    Mock::given(method("PATCH"))
        .and(path("/api/environments/999999/dashboards/30/move_tile/"))
        .and(body_partial_json(json!({
            "tile": { "id": 5, "layouts": { "sm": {"x":0,"y":0,"w":6,"h":5} } }
        })))
...
// ── 9. tiles reorder: hits /reorder_tiles/ endpoint with {tile_order} ─────────

#[tokio::test]
async fn dashboard_tiles_reorder_calls_reorder_tiles_endpoint() {
    use wiremock::matchers::body_partial_json;
    ...
    // Accept the object-form (back-compat) as well as bare-array form.
    std::fs::write(&order_file, r#"[{"id": 1}, {"id": 2}, {"id": 3}]"#).unwrap();
...
// ── 10. tiles copy: hits /copy_tile/ on destination with {fromDashboardId,tileId}

#[tokio::test]
async fn dashboard_tiles_copy_calls_copy_tile_endpoint() {
    use wiremock::matchers::body_partial_json;

    let h = TestHarness::new().await;

    Mock::given(method("POST"))
        .and(path("/api/environments/999999/dashboards/99/copy_tile/"))
        .and(body_partial_json(json!({
            "fromDashboardId": 50,
            "tileId": 7
        })))
...
// ── 10b. tiles remove: GET dashboard → PATCH insight.dashboards minus id ──────
